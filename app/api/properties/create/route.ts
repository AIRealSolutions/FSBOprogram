...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data') && (photos.length || documents.length || videoFile || body.intakeAnswers)) {
      ...
      try {
        // Upload answer sheet as JSON (even if empty, it helps keep the intake folder consistent).
    ...
    return NextResponse.json({ ok: true, property, ownerMode: authedUser ? 'auth' : 'default', warnings });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
      {
